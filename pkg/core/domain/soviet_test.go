...
func TestSovietState_RegisterAgent(t *testing.T) {
	...
	// Register agent
	err := soviet.RegisterAgent(agent)
	assert.NoError(t, err)
	...
	err = soviet.RegisterAgent(duplicateAgent)
	assert.Error(t, err)
...
func TestSovietState_GetAgentRoles(t *testing.T) {
	...
	soviet.RegisterAgent(agent1)
	soviet.RegisterAgent(agent2)
